...
##########read counts转化成TPM#############################################################
allsamples_counts<-read.csv("allsamples_readcounts_anno.csv", header = T, row.names = 1)
kb<-allsamples_counts$Length/1000
rpk <- allsamples_counts[2:12] / kb
rpk
tpm <- t(t(rpk)/colSums(rpk) * 1000000)
...
############merged and cluster#############################################################
setwd("/Users/liuzhe/Desktop/UCSF/milk/analysis1/six_comparision_addIandJ/merged_heatmap")
exp_deg_fat_cell<-read.csv("/Users/liuzhe/Desktop/UCSF/milk/analysis1/six_comparision_addIandJ/Fat_Cell/Table1_Fat-cell.csv",
                           header = T, row.names = 1)
exp_deg_fat_breast<-read.csv("/Users/liuzhe/Desktop/UCSF/milk/analysis1/six_comparision_addIandJ/Fat_Breast/Table2_Fat-breast.csv", 
                             header = T, row.names = 1)
exp_deg_cell_breast<-read.csv("/Users/liuzhe/Desktop/UCSF/milk/analysis1/six_comparision_addIandJ/Cell_Breast/Table3_Cell-breast.csv", 
                              header = T, row.names = 1)
exp_deg_fatcell_breast<-read.csv("/Users/liuzhe/Desktop/UCSF/milk/analysis1/six_comparision_addIandJ/FatCell_Breast/Table4_FatCell-breast.csv",
                                 header = T, row.names = 1)
exp_deg_fatbreast_cell<-read.csv("/Users/liuzhe/Desktop/UCSF/milk/analysis1/six_comparision_addIandJ/FatBreast_Cell/Table5_FatBreast-cell.csv",
                                 header = T, row.names = 1)
exp_deg_cellbreast_fat<-read.csv("/Users/liuzhe/Desktop/UCSF/milk/analysis1/six_comparision_addIandJ/CellBreast_Fat/Table6_CellBreast-fat.csv",
                                 header = T, row.names = 1)
deg_all<-unique(c(row.names(exp_deg_fat_cell), row.names(exp_deg_fat_breast), row.names(exp_deg_cell_breast),
                  row.names(exp_deg_fatcell_breast),row.names(exp_deg_fatbreast_cell),row.names(exp_deg_cellbreast_fat)))

length(deg_all)
#[1] 12481

tpm<-read.csv("/Users/liuzhe/Desktop/UCSF/milk/analysis1/six_comparision_addIandJ/tpm.csv", header = T, row.names = 1)
overlap_genes<-intersect(rownames(tpm),deg_all)
tpm_selected<-tpm[overlap_genes,]
sample_info<-read.csv("/Users/liuzhe/Desktop/UCSF/milk/analysis1/addSample_IJ/sample_information.csv", header = T)

library("pheatmap")
annotation_col = data.frame(Type = factor(sample_info$Type), Sample = factor(sample_info$Donor))
rownames(annotation_col) = colnames(tpm_selected)
ann_colors = list(Type = c("Cells" = "green", "Fat" = "red", "NormalBreast" = "blue"),
                  Sample = c("1012" = "green", "1013" = "orange","1014" = "purple", "B1" = "yellow",
                             "B2" = "blue", "B3" = "pink"))
tpm_selected_resort<-subset(tpm_selected, select = c("B", "D", "F", "J", "A", "C", "E", "I",
                                                     "SRR2148259.sra", "SRR2148260.sra", "SRR2148261"))

#########################kmeans, k=2########################################################################
setwd("/Users/liuzhe/Desktop/UCSF/milk/analysis1/six_comparision_addIandJ/kmeans/k=2")
p<-pheatmap(tpm_selected_resort, scale = "row", clustering_distance_rows = "correlation", cluster_rows = F, cluster_col = F,
            color = colorRampPalette(c("blue", "white", "red"))(50), annotation_col = annotation_col,
            angle_col = "45", annotation_colors = ann_colors, kmeans_k = 2,
            main = "Title")
...
p
dev.off()
save(p, file = "pheatmap_kmeans_2.RData")
...
write.csv(cluster_1, "cluster_1_FatCell.csv", quote = F)
cluster_2<-subset(cluster_results, cluster_results == "2")
write.csv(cluster_2, "cluster_2_Breast.csv", quote = F)
...
genes_sorted<-c(rownames(cluster_1), rownames(cluster_2))
tpm_selected_resort<-tpm_selected_resort[genes_sorted,]
pdf(file="heatmap_kmeans_2_gene.pdf",width = 6,height = 6)
...
tpm_selected_show_50<-tpm_selected_resort[c(1:25,7838:7862),]
pdf(file="heatmap_kmeans_merged_gene_show50.pdf",width = 6,height = 8)
pheatmap(tpm_selected_show_50, scale = "row", clustering_distance_rows = "correlation", cluster_rows = F, 
         cluster_col = F,color = colorRampPalette(c("blue", "white", "red"))(50), annotation_col = annotation_col,
...
go_BP <- enrichGO(test$ENTREZID, 
                  ...
                  qvalueCutoff = 1,
                  keyType = 'ENTREZID')
write.csv(summary(go_BP),"GOBP_enrich_cluster1_FatCell-Breast.csv",row.names =FALSE)
y <- setReadable(go_BP, OrgDb = org.Hs.eg.db, keyType="ENTREZID")
### The geneID column is translated to symbol
write.csv(y,"GOBP_enrich_cluster1_FatCell-Breast_readable.csv",row.names =FALSE)
#结果可视化
GOBP<-summary(go_BP)
GOBP_filter<-subset(GOBP, pvalue < 0.05)
#GOBP_filter_sort<-GOBP_filter[order(-GOBP_filter$Count),]
GOBP_select<-GOBP_filter[1:20,]
GOBP_select$yvalue<-GOBP_select$Count/3852
GOBP_select$xvalue<-GOBP_select$Description
library(ggpubr)
pdf("GOBP_cluster1_FatCell.barplot.pdf",height=6,width = 12)
...
go_BP <- enrichGO(test$ENTREZID, 
                  ...
                  qvalueCutoff = 1,
                  keyType = 'ENTREZID')
write.csv(summary(go_BP),"GOBP_enrich_cluster2_Breast.csv",row.names =FALSE)
y <- setReadable(go_BP, OrgDb = org.Hs.eg.db, keyType="ENTREZID")
### The geneID column is translated to symbol
write.csv(y,"GOBP_enrich_cluster2_Breast_readable.csv",row.names =FALSE)
#结果可视化
GOBP<-summary(go_BP)
GOBP_filter<-subset(GOBP, pvalue < 0.05)
#GOBP_filter_sort<-GOBP_filter[order(-GOBP_filter$Count),]
GOBP_select<-GOBP_filter[1:20,]
GOBP_select$yvalue<-GOBP_select$Count/5396
GOBP_select$xvalue<-GOBP_select$Description
library(ggpubr)
pdf("GOBP_cluster2_Breast.barplot.pdf",height=6,width = 12)
ggbarplot(GOBP_select, x = "xvalue", y = "yvalue", orientation = "horiz",
          xlab = "", ylab="Gene Ratio",
          fill = "pvalue",
          color = "white",
...
#########################kmeans, k=5########################################################################
setwd("/Users/liuzhe/Desktop/UCSF/milk/analysis1/six_comparision_addIandJ/kmeans/k=5")
annotation_col = data.frame(Type = factor(sample_info$Type), Sample = factor(sample_info$Donor))
rownames(annotation_col) = c("A", "B", "C", "D", "E", "F", "I", "J",
                             "SRR2148259.sra", "SRR2148260.sra", "SRR2148261")

p<-pheatmap(tpm_selected_resort, scale = "row", clustering_distance_rows = "correlation", cluster_rows = F, cluster_col = F,
            color = colorRampPalette(c("blue", "white", "red"))(50), annotation_col = annotation_col,
            angle_col = "45", annotation_colors = ann_colors, kmeans_k = 5,
            main = "Title")
...
p
dev.off()
save(p, file = "pheatmap_kmeans_5.RData")
cluster_results<-p$kmeans$cluster
cluster_results<-as.data.frame(cluster_results)
cluster_1<-subset(cluster_results, cluster_results == "1")
...
genes_sorted<-c(rownames(cluster_1), rownames(cluster_2), rownames(cluster_3), 
                rownames(cluster_4), rownames(cluster_5))
tpm_selected_resort<-tpm_selected_resort[genes_sorted,]
pdf(file="heatmap_kmeans_5_gene.pdf",width = 6,height = 6)
pheatmap(tpm_selected_resort, scale = "row", clustering_distance_rows = "correlation", cluster_rows = F, 
         cluster_col = F,color = colorRampPalette(c("blue", "white", "red"))(50), annotation_col = annotation_col,
         angle_col = "45", annotation_colors = ann_colors, main = "Title")
dev.off()
tpm_selected_show_100<-tpm_selected_resort[c(1:20,2268:2287,5117:5136,6222:6241,7785:7804),]
pdf(file="heatmap_kmeans_merged_gene_show100.pdf",width = 6,height = 8)
pheatmap(tpm_selected_show_100, scale = "row", clustering_distance_rows = "correlation", cluster_rows = F, 
         cluster_col = F,color = colorRampPalette(c("blue", "white", "red"))(50), annotation_col = annotation_col,
         angle_col = "45", annotation_colors = ann_colors, main = "Title", fontsize_row = 4 )
dev.off()
...
#GOBP_filter_sort<-GOBP_filter[order(-GOBP_filter$Count),]
GOBP_select<-GOBP_filter[1:20,]
GOBP_select$yvalue<-GOBP_select$Count/1870
GOBP_select$xvalue<-GOBP_select$Description
library(ggpubr)
...
#GOBP_filter_sort<-GOBP_filter[order(-GOBP_filter$Count),]
GOBP_select<-GOBP_filter[1:20,]
GOBP_select$yvalue<-GOBP_select$Count/2016
GOBP_select$xvalue<-GOBP_select$Description
library(ggpubr)
...
#GOBP_filter_sort<-GOBP_filter[order(-GOBP_filter$Count),]
GOBP_select<-GOBP_filter[1:20,]
GOBP_select$yvalue<-GOBP_select$Count/877
GOBP_select$xvalue<-GOBP_select$Description
library(ggpubr)
pdf("GOBP_cluster.barplot.pdf",height=6,width = 12)
ggbarplot(GOBP_select, x = "xvalue", y = "yvalue", orientation = "horiz",
          xlab = "", ylab="Gene Ratio",
          fill = "pvalue",
          color = "white",
...
#GOBP_filter_sort<-GOBP_filter[order(-GOBP_filter$Count),]
GOBP_select<-GOBP_filter[1:20,]
GOBP_select$yvalue<-GOBP_select$Count/1333
GOBP_select$xvalue<-GOBP_select$Description
library(ggpubr)
...
#GOBP_filter_sort<-GOBP_filter[order(-GOBP_filter$Count),]
GOBP_select<-GOBP_filter[1:20,]
GOBP_select$yvalue<-GOBP_select$Count/3152
GOBP_select$xvalue<-GOBP_select$Description
library(ggpubr)
...
#########################kmeans, k=6########################################################################
setwd("/Users/liuzhe/Desktop/UCSF/milk/analysis1/six_comparision_addIandJ/kmeans/k=6")
p<-pheatmap(tpm_selected_resort, scale = "row", clustering_distance_rows = "correlation", cluster_rows = F, cluster_col = F,
            color = colorRampPalette(c("blue", "white", "red"))(50), annotation_col = annotation_col,
...
tpm_selected_show_150<-tpm_selected_resort[c(1:25,2497:2521,3449:3473,5821:5845,8914:8939,10261:10285),]
pdf(file="heatmap_kmeans_merged_gene_show150.pdf",width = 6,height = 8)
pheatmap(tpm_selected_show_150, scale = "row", clustering_distance_rows = "correlation", cluster_rows = F, 
         cluster_col = F,color = colorRampPalette(c("blue", "white", "red"))(50), annotation_col = annotation_col,
         angle_col = "45", annotation_colors = ann_colors, main = "Title", fontsize_row = 4 )
dev.off()
...
#GOBP_filter_sort<-GOBP_filter[order(-GOBP_filter$Count),]
GOBP_select<-GOBP_filter[1:20,]
GOBP_select$yvalue<-GOBP_select$Count/2065
GOBP_select$xvalue<-GOBP_select$Description
library(ggpubr)
pdf("GOBP_cluster1.barplot.pdf",height=6,width = 12)
ggbarplot(GOBP_select, x = "xvalue", y = "yvalue", orientation = "horiz",
...
#GOBP_filter_sort<-GOBP_filter[order(-GOBP_filter$Count),]
GOBP_select<-GOBP_filter[1:20,]
GOBP_select$yvalue<-GOBP_select$Count/660
GOBP_select$xvalue<-GOBP_select$Description
library(ggpubr)
pdf("GOBP_cluster2.barplot.pdf",height=6,width = 12)
ggbarplot(GOBP_select, x = "xvalue", y = "yvalue", orientation = "horiz",
...
#GOBP_filter_sort<-GOBP_filter[order(-GOBP_filter$Count),]
GOBP_select<-GOBP_filter[1:20,]
GOBP_select$yvalue<-GOBP_select$Count/1961
GOBP_select$xvalue<-GOBP_select$Description
library(ggpubr)
pdf("GOBP_cluster.barplot.pdf",height=6,width = 12)
ggbarplot(GOBP_select, x = "xvalue", y = "yvalue", orientation = "horiz",
...
#GOBP_filter_sort<-GOBP_filter[order(-GOBP_filter$Count),]
GOBP_select<-GOBP_filter[1:20,]
GOBP_select$yvalue<-GOBP_select$Count/2198
GOBP_select$xvalue<-GOBP_select$Description
library(ggpubr)
pdf("GOBP_cluster4.barplot.pdf",height=6,width = 12)
ggbarplot(GOBP_select, x = "xvalue", y = "yvalue", orientation = "horiz",
...
#GOBP_filter_sort<-GOBP_filter[order(-GOBP_filter$Count),]
GOBP_select<-GOBP_filter[1:20,]
GOBP_select$yvalue<-GOBP_select$Count/965
GOBP_select$xvalue<-GOBP_select$Description
library(ggpubr)
pdf("GOBP_cluster5.barplot.pdf",height=6,width = 12)
ggbarplot(GOBP_select, x = "xvalue", y = "yvalue", orientation = "horiz",
...
#GOBP_filter_sort<-GOBP_filter[order(-GOBP_filter$Count),]
GOBP_select<-GOBP_filter[1:20,]
GOBP_select$yvalue<-GOBP_select$Count/1399
GOBP_select$xvalue<-GOBP_select$Description
library(ggpubr)
